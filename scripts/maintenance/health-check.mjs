...
const validateNestedRepository = async ({ commandExecutor, root, stderr }) => {
  ...
  if (path.resolve(detectedRoot) !== root) {
    stderr(
      `FAIL inspect nested Git repository: unexpected top-level path ${detectedRoot}`,
    );
    return false;
  }
...
const validateCanonicalSource = async ({ commandExecutor, root, stderr }) => {
  ...
  const untracked = discovered.filter(
    (relativePath) => !trackedPaths.has(relativePath),
  );
  ...
  if (missing.length > 0) {
    stderr(
      `FAIL tracked source is missing or not a file:\n${missing.join("\n")}`,
    );
    return false;
  }
...
const validateLocalPaths = async ({
  commandExecutor,
  instructionRoutes,
  root,
  stderr,
  ...
  const localInstructionFiles = await collectFiles(
    root,
    "instructions",
  ...
  const trackedLocalPaths = tracked.paths.filter(
    (relativePath) =>
      ...
      LOCAL_ONLY_ROOTS.some(
        (localRoot) =>
          relativePath === localRoot ||
          relativePath.startsWith(`${localRoot}/`),
      ),
  );
...
const readInstructionRoutes = async ({ root, stderr }) => {
  ...
  const afterHeading = indexSource.slice(
    rulesHeading.index + rulesHeading[0].length,
  );
  const nextSection = afterHeading.search(/^## /m);
  const rulesSource =
    nextSection < 0 ? afterHeading : afterHeading.slice(0, nextSection);
  const loadBullets = [];
  let currentBullet;
...
const validateInstructionRoutes = async ({
  instructionRoutes,
  root,
  stderr,
}) => {
  ...
  if (missing.size === 0) return true;
  stderr(
    `FAIL missing relative instruction routes:\n${[...missing].sort().join("\n")}`,
  );
  return false;
};

const validateParentIndex = async ({
  commandExecutor,
  root,
  stdout,
  stderr,
  ...
  if (path.resolve(detectedRoot) !== parentRoot) {
    stderr(
      `FAIL inspect parent Git repository: unexpected top-level path ${detectedRoot}`,
    );
    return false;
  }
...
export const runHealthCheck = async ({
  ...
  if (options.full) {
    const testFiles = await collectFiles(root, "scripts", (file) =>
