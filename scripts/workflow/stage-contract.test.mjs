...
test("pull request creation is explicit and summary-only", async () => {
  ...
  assert.match(
    prompt,
    /Never commit, amend, rebase, squash, force-push, merge, close/,
  );
  assert.doesNotMatch(
    prompt,
    /^## (Validation|Risk|Migrations|Deferred Checks)$/m,
  );
...
test("MEDIUM and HIGH require an independent final review loop", async () => {
  ...
  assert.match(
    review,
    /rounds 1 and 2[\s\S]*automatically start the next fresh review round[\s\S]*true external or missing-input blocker/,
  );
  ...
  assert.match(
    checkpoint,
    /When no stable component scope exists[\s\S]*fix: <imperative summary/,
  );
  assert.match(checkpoint, /Its body must state the review round/);
  assert.match(
    checkpoint,
    /summaries of every resolved `P0`\/`P1`\/`P2` finding/,
  );
  assert.match(checkpoint, /validation\s+commands\/results/);
  assert.doesNotMatch(checkpoint, /use\s+`fix\(review\):/);
  assert.match(
    checkpoint,
    /blocking rounds 1 and 2[\s\S]*fresh independent final-review round[\s\S]*blocking round 3[\s\S]*Awaiting operator decision/,
  );
  assert.match(
    workflow,
...
test("final review continuation checkpoints bound automatic remediation rounds", async () => {
  const [agents, workflow, execute, review, checkpoint, readme, usage] =
    await Promise.all([
      readSource("AGENTS.md"),
      readSource("instructions/shared/workflow-state.md"),
      readSource("prompts/execute-plan.md"),
  ...
  const normalizedCheckpoint = checkpoint.replace(/\s+/g, " ");

  assert.match(
    review,
    /A review round is counted only when a fresh independent reviewer returns/,
  );
  assert.match(
    review,
    /rounds 1 and 2[\s\S]*automatically start the next fresh review round/i,
  );
  assert.match(review, /blocking round 3[\s\S]*Awaiting operator decision/);
  assert.match(normalizedReview, /pause before spawning the next reviewer/i);
  assert.match(review, /exact standalone, case-sensitive\s+token/);
  const tokenSection =
    /10\. At an active checkpoint,[\s\S]*?token:\n([\s\S]*?)\n11\./.exec(
      review,
    )?.[1];
  assert.ok(
    tokenSection,
    "canonical active-checkpoint token section is present",
  );
  assert.deepEqual(
    [...tokenSection.matchAll(/^\s{4}- `([^`]+)`/gm)].map((match) => match[1]),
    ["END_REVIEW", "REVIEW_NEXT_ROUND", "REVIEW_UNTIL_CLEAR"],
  );
  assert.match(
    normalizedReview,
    /only three accepted tokens.*No token or authorization is inferred, enabled by default, or accepted outside an active checkpoint/i,
  );
  assert.match(
    normalizedReview,
    /`REVIEW_NEXT_ROUND`.*exactly one additional fresh cumulative review round/,
  );
  assert.match(
    normalizedReview,
    /REVIEW_NEXT_ROUND.*blocking.*new continuation checkpoint/i,
  );
  assert.match(
    normalizedReview,
    /REVIEW_NEXT_ROUND.*consumed only when that reviewer returns a report.*Authorization active: no.*Authorized next round: None/i,
  );
  assert.match(
    normalizedReview,
    /REVIEW_NEXT_ROUND.*reviewer runtime or required review evidence fails before a report returns.*do not increment the round or consume.*retry that same authorized round.*without requiring a new checkpoint or token/i,
  );
  assert.match(
    normalizedReview,
    /`REVIEW_UNTIL_CLEAR`.*persist.*automatically start.*fresh review round/i,
  );
  assert.match(
    normalizedReview,
    /REVIEW_UNTIL_CLEAR.*without creating another continuation checkpoint/i,
  );
  assert.match(
    normalizedReview,
    /clear round.*Deactivate any authorization.*Authorized next round.*None.*retain the operator token and selected mode/i,
  );
  assert.match(
    normalizedReview,
    /REVIEW_UNTIL_CLEAR.*stops.*Deactivate the authorization.*Authorized next round.*None.*new eligible continuation checkpoint.*never reactivate/i,
  );
  assert.match(normalizedReview, /`END_REVIEW`.*`Completed by operator`/);
  assert.match(
    review,
    /latest remediation\s+was not independently re-reviewed/,
  );
  assert.match(
    review,
    /invalid[\s\S]*leaves the status at `Awaiting operator decision`/i,
  );
  assert.match(review, /stale[\s\S]*has no\s+review-control effect/i);
  assert.match(
    review,
    /required validation failure[\s\S]*must not enter `Awaiting operator decision`/i,
  );
  for (const stopCondition of [
    "incomplete remediation",
    "required validation fails",
    "reviewer runtime fails",
    ...
    assert.match(
      normalizedReview,
      new RegExp(`REVIEW_UNTIL_CLEAR[\\s\\S]*${stopCondition}`, "i"),
      stopCondition,
    );
  }
  assert.match(
  ...
  assert.match(review, /round number[\s\S]*checkpoint[\s\S]*operator token/i);
  assert.match(
    review,
    /Selected authorization: <None \| one-round \| until-clear>/,
  );

  assert.match(execute, /Awaiting operator decision/);
  assert.match(execute, /Completed by operator/);
  assert.match(execute, /END_REVIEW/);
  assert.match(execute, /REVIEW_NEXT_ROUND/);
  assert.match(execute, /REVIEW_UNTIL_CLEAR/);
  assert.match(
    normalizedCheckpoint,
    /blocking round 3.*Awaiting operator decision.*after every applicable HIGH remediation commit/i,
  );
  assert.match(
    normalizedCheckpoint,
    /REVIEW_NEXT_ROUND.*exactly one fresh final-review round/,
  );
  assert.match(
    normalizedCheckpoint,
    /REVIEW_UNTIL_CLEAR.*applicable HIGH remediation commit.*automatically/i,
  );
  assert.match(checkpoint, /Completion path:\s+reviewer-cleared/);
  assert.match(checkpoint, /Completion path: operator-ended/);
  assert.match(
    review,
    /explicit `reviewer-cleared`\s+or `operator-ended` completion path/,
  );
  assert.match(workflow, /five MEDIUM review statuses/i);
  assert.match(workflow, /rounds 1 and 2[\s\S]*automatically/i);
  assert.match(
    agents.replace(/\s+/g, " "),
    /after remediated round 3.*continuation checkpoint/i,
  );

  assert.match(
    review,
    /clear round completes as `Ready to complete` without a continuation\s+checkpoint/i,
  );
  assert.match(review, /`P3`[\s\S]*without creating a continuation checkpoint/);
  assert.match(
    review,
    /`Blocked` only for a true external or missing-input blocker/i,
  );
  assert.match(
    review,
    /LOW self-checks[\s\S]*HIGH task reviews[\s\S]*unchanged/i,
  );
  ...
  for (const source of [
    ...
    assert.doesNotMatch(source, /CONTINUE_REVIEW/);
  }
  for (const documentation of [readme, usage]) {
    assert.match(documentation, /three default automatic\s+rounds/i);
    assert.match(documentation, /END_REVIEW/);
    assert.match(documentation, /REVIEW_NEXT_ROUND/);
    assert.match(documentation, /REVIEW_UNTIL_CLEAR/);
    assert.match(documentation, /reviewer|validation|remediation/i);
    assert.match(documentation, /operator-ended/i);
  }
  assert.match(usage, /explicitly invoked execution stage/);
  assert.match(usage, /does not[\s\S]*authorize delivery/);
});
...
test("local AGENTS override setup delegates to one deterministic utility", async () => {
  const [packageSource, prompt, readme, codexAgent, utility] =
    await Promise.all([
...
Use \`.ai/instructions/index.md\` to load only instructions relevant to the request.`;
  ...
  assert.match(
    prompt,
    /delegate[s]? all mutation and validation to the package utility/i,
  );
