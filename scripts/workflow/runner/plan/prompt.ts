...
Run \`pnpm lint-staged\` once and wait or poll that same command until it exits; do not start a second lint or commit command while it is still running. If its terminal session closes or loses stdin before reporting an exit status, first confirm no lint-staged or ESLint child remains, then rerun \`pnpm lint-staged\` once in a fresh terminal session and wait for its confirmed result. Do not output \`STOP\` solely for a stale terminal session; output it only for a confirmed nonzero fresh run or an active process that cannot be safely observed. If the terminal command record reports that \`pnpm lint-staged\` exited with code \`0\`, treat that as confirmed success even when the interactive terminal subsequently loses stdin: restage and continue without a duplicate lint or \`STOP\`. After it succeeds and the same paths are restaged, \`SKIP_LINT_STAGED=1\` on the final commit skips only the duplicate formatter/linter pass; the pre-commit hook still enforces branch, ignored-file, environment-file, and secret checks. Do not use \`HUSKY=0\` or \`--no-verify\`.
